...
_Last updated: 2026-05-25. The N/A / MISSING / VACUUM signals are logged here so
...
- **Logos tetrad incomplete**: `pragma.md` + this `praxis.md` now exist;
  `telos.md` (idealized form), `receptio.md` (reception), and
...
- **Auth0 must issue an `org_id` claim in production** — the IDOR fix derives org
  ...
  middleware already accepts namespaced Auth0 claims.
- **RLS requires a non-owner DB role** — policies are not `FORCE`d; the table
  ...
  wired via `orgContextMiddleware`.
- **Plaid** ES256/JWK verification is implemented; key-rotation/cache refresh
  policy can be hardened further.
- **dataTier** entitlement now reads `organizations.subscription_tier`; confirm
  the tier→data-tier mapping matches product intent.
- **Per-tenant job scoping** is gated to `admin` + org-filtered; BullMQ jobs now
  carry `orgId` — revisit when a richer job-ownership model exists.
