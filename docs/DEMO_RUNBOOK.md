...
| `PLAID_CLIENT_ID`, `PLAID_SECRET` (`PLAID_ENV`)                               | `server/integrations/plaid/client.ts`; `server/middleware/webhookAuth.ts`                                  | Real bank-data extraction for `POST /api/prospects/:id/underwrite` and verified Plaid webhooks via ES256/JWK lookup. Without credentials, underwriting and Plaid webhooks fail closed.                                                                                                                                                                                                                                             | Plaid production keys                   |
...
startup unless `STRIPE_WEBHOOK_SECRET`, `TWILIO_AUTH_TOKEN`, `PLAID_CLIENT_ID`, `PLAID_SECRET` and
